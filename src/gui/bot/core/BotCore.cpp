    ...
    : QObject(parent)
{
    ...
    try {
        ...
        LogManager::instance().error(
            QString("Failed to initialize MemoryManager: %1").arg(e.what()),
            "Core",
            "Memory"
        );
...
bool BotCore::initialize()
{
    ...
    // Устанавливаем хуки
    if (!setupHooks()) {
        QString error = QString("Failed to set up hooks for process %1. Memory access denied?").arg(m_context.processId);
    ...
    m_initialized = true;
    LogManager::instance().info(
        QString("BotCore initialized for process: %1, window: 0x%2")
...
namespace {
    ...
    bool isWoWWindow(const char* className, const char* windowTitle) {
        ...
        return (strcmp(className, "GxWindowClassD3d") == 0 || 
    ...
    BOOL CALLBACK enumWindowsProc(HWND hwnd, LPARAM lParam) {
        ...
        if (windowPid == data->targetPid) {
            ...
            LogManager::instance().debug(
                QString("Found window - PID: %1, Class: %2, Title: %3")
                    .arg(windowPid)
                    .arg(className)
                    .arg(windowTitle),
                "Core"
...
bool BotCore::findWindowHandle()
{
    ...
    if (data.result) {
        ...
        if (!IsWindow(data.result) || !IsWindowVisible(data.result)) {
            LogManager::instance().error(
                QString("Window handle 0x%1 for process %2 is invalid or not visible")
        ...
        LogManager::instance().info(
            QString("Found WoW window handle: 0x%1 for process %2")
                .arg(QString::number((quintptr)data.result, 16))
                .arg(m_context.processId),
            "Core"
        );
    ...
    LogManager::instance().error(
        QString("Failed to find WoW window for process %1. Make sure the game window is open.")
            .arg(m_context.processId),
        "Core",
        "Window"
    );
...
bool BotCore::setupHooks()
{
    ...
    LogManager::instance().debug(
        QString("Found run.exe base address: 0x%1")
    ...
    LogManager::instance().debug(
        QString("Calculated hook target address: 0x%1 (base: 0x%2 + offset: 0x%3)")
    ...
    if (!m_memory->IsValidAddress(targetAddress)) {
        LogManager::instance().error(
            QString("Invalid hook target address: 0x%1")
                .arg(QString::number(targetAddress, 16)),
            "Core",
            "Hooks"
        );
    ...
    try {
        ...
        LogManager::instance().debug(
            QString("Current memory protection at 0x%1: 0x%2")
                .arg(QString::number(targetAddress, 16))
        ...
        if (!(currentProtection & PAGE_EXECUTE_READWRITE)) {
            ...
            if (!m_memory->SetMemoryProtection(targetAddress, 5, PAGE_EXECUTE_READWRITE)) {
                DWORD error = GetLastError();
                LogManager::instance().error(
                    QString("Failed to change memory protection. Error: %1")
                        .arg(error),
                    "Core",
                    "Hooks"
                );
            ...
            LogManager::instance().debug(
                QString("Memory protection after change at 0x%1: 0x%2")
                    .arg(QString::number(targetAddress, 16))
        ...
        if (success) {
            LogManager::instance().info(
                QString("Successfully installed hook at address: 0x%1")
                    .arg(QString::number(targetAddress, 16)),
                "Core"
            );
        } else {
            LogManager::instance().error(
                QString("Failed to install hook at address: 0x%1")
                    .arg(QString::number(targetAddress, 16)), 
                "Core", 
                "Hooks"
            );
        ...
        LogManager::instance().error(
            QString("Exception while setting up hooks: %1").arg(e.what()),
            "Core",
            "Hooks"
        );
        return false;
        ...
        try {
            ...
            if (!m_memory->IsValidAddress(regs.eax)) {
                LogManager::instance().warning(
                    QString("Invalid EAX address: 0x%1").arg(QString::number(regs.eax, 16)), 
                    "Core", "Memory"
                );
                return;
            }

            LogManager::instance().debug(QString("Updating character data from EAX: 0x%1").arg(QString::number(regs.eax, 16)), "Core");

            // Читаем данные персонажа из памяти
            m_context.character.currentHealth = m_memory->Read<uint32_t>(
                regs.eax + CharacterData::CURRENT_HP_OFFSET
            );
            
            m_context.character.currentMana = m_memory->Read<uint32_t>(
                regs.eax + CharacterData::CURRENT_MANA_OFFSET
            );
            
            m_context.character.maxHealth = m_memory->Read<uint32_t>(
                regs.eax + CharacterData::MAX_HP_OFFSET
            );
            
            m_context.character.maxMana = m_memory->Read<uint32_t>(
                regs.eax + CharacterData::MAX_MANA_OFFSET
            );
            
            m_context.character.level = m_memory->Read<uint32_t>(
                regs.eax + CharacterData::LEVEL_OFFSET
            );

            m_context.character.eaxRegister = regs.eax;
            ...
            LogManager::instance().error(QString("Error updating character data: %1").arg(e.what()), "Core", "Memory");
        }
    }
}